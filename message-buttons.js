...
// Add one host-style action beside each rendered AI reply. A small observer is
// scoped to #chat so upward history loading and message redraws are covered
// without repeatedly scanning the whole page.
export function attachMessageButtons(c, ui, root = document) {
  ...
  const ensure = messageElement => {
    ...
    action.innerHTML = '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M12 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.4 2.6a2.1 2.1 0 0 1 3 3l-9 9a2 2 0 0 1-.9.5l-2.9.9a.5.5 0 0 1-.6-.6l.9-2.9a2 2 0 0 1 .5-.9z"/></svg>';
    action.title = '检测本楼层';
    action.setAttribute('aria-label', '检测本楼层');
    action.disabled = c.settings().enabled === false;
    edit.before(action);
  };
  const scanNode = node => {
  ...
  ui.onPluginAvailabilityChange = () => chat?.querySelectorAll(`.${BUTTON_CLASS}`).forEach(action => { action.disabled = c.settings().enabled === false; });
