...
export function renderRulesView(all, f, ruleId, form, d, execution = 'review') {
  ...
  return `<div class="tr-bar"><span>${rules.length} / ${all.length} 条规则</span><div class="tr-rule-tools">${button('＋ 新增', 'data-action="new-rule"')}${hostIcon('file-import', '导入 JSON 规则集', 'import-rules')}${hostIcon('file-export', '导出 JSON 规则集', 'export-rules')}</div></div><input id="tr-rule-import-file" type="file" accept=".json,application/json" hidden>
