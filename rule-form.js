...
export function renderRuleForm(draft) {
  ...
  return `<form id="tr-rule-form" class="tr-rule-editor">
    ...
    <p class="tr-meta">直接填正则，默认匹配全部，不用加 /…/g。纯词表可用英文逗号或换行分隔。</p>
    ...
    <p class="tr-meta" id="tr-replacement-help">${draft.wholeReplacement ? '整框是一份替换内容，标点、空格和换行原样保留。留空删除。' : '英文逗号或换行分隔多个候选，每处随机选一个；只有一个就固定替换，留空删除。中文逗号保留。'}</p>
